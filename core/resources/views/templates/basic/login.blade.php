            ...
            <div class="form-wrapper">
                ...
                <form method="POST" class="cmn-form mt-30 route">
                    ...
                    <div class="form-group">
                        ...
                        <select name="" id="access" class="form-control b-radius--capsule">
                            <option value="" data-route="{{ route('doctor.login') }}"
                                    data-forget="{{ route('doctor.password.reset') }}">@lang('Mentor')</option>
                            <!--<option value="" data-route="{{ route('assistant.login') }}"-->
                            <!--        data-forget="{{ route('assistant.password.reset') }}">@lang('Assistant')</option>-->
                            <!--<option value="" data-route="{{ route('staff.login') }}"-->
                            <!--        data-forget="{{ route('staff.password.reset') }}">@lang('Staff')</option>-->

                            <option value="" data-route="{{ route('user.login') }}"
                                    data-forget="{{ route('user.password.reset') }}">@lang('Mentee')</option>
                        </select>
                    ...
                    <div class="form-group d-flex justify-content-between align-items-center">
                        <a class="text-muted text--small forget"><i class="las la-lock"></i>@lang('Forgot password?')
                        </a>
                    </div>
                    <div class="form-group">
                        <button type="submit" class="submit-btn mt-25 b-radius--capsule">@lang('Login') <i
                                class="las la-sign-in-alt"></i></button>
                        <a href="{{route('register')}}" class="submit-btn mt-25 b-radius--capsule">@lang('Register') <i
                                class="las la-sign-in-alt"></i></a>

                        <a href="auth/linkedin" class="btn btn-primary linkedInButton">@lang('Login with LinkedIn') <i
                                class="las la-sign-in-alt"></i></a>
                                 <a href="auth/google" class="btn btn-warning linkedInButton">@lang('Login with Google') <i
                                class="las la-sign-in-alt"></i></a>
                    </div>
                </form>
            </div>
