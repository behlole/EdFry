                    ...
                    <div class="table-responsive table-responsive--sm">
                        <table class="default-data-table table">
                            ...
                            <tr>
                                ...
                                <th scope="col">@lang('Start URL')</th>
                                <th scope="col">@lang('Join URL')</th>
                            ...
                            @foreach($appointments as $item)
                                <tr>
                                    ...
                                    @if ($item->staff)
                                        <td data-label="@lang('Added By')">{{ $item->relationStaff->name }} <span
                                                class="text--small badge font-weight-normal badge--success">@lang('Staff')</span>
                                        </td>
                                    @elseif($item->assistant)
                                        <td data-label="@lang('Added By')">{{ $item->relationAssistant->name }} <span
                                                class="text--small badge font-weight-normal badge--success">@lang('Assistant')</span>
                                        </td>
                                    @elseif($item->entry_doctor)
                                        <td data-label="@lang('Added By')">{{ $item->doctor->name }} <span
                                                class="text--small badge font-weight-normal badge--success">@lang('Doctor')</span>
                                        </td>
                                    @elseif($item->admin)
                                        <td data-label="@lang('Added By')">{{ $item->relationAdmin->name }} <span
                                                class="text--small badge font-weight-normal badge--success">@lang('Admin')</span>
                                        </td>
                                    @elseif($item->site)
                                        <td data-label="@lang('Added By')">@lang('From Site') <span
                                                class="text--small badge font-weight-normal badge--success">@lang('Site')</span>
                                        </td>
                                    @else
                                    ...
                                    @if ($item->p_status == 0)
                                        <td data-label="@lang('Payment Status')"><span
                                                class="text--small badge font-weight-normal badge--danger">@lang('Pay in cash')</span>
                                        </td>
                                    @elseif ($item->p_status == 1)
                                        <td data-label="@lang('Payment Status')"><span
                                                class="text--small badge font-weight-normal badge--success">@lang('Paid')</span>
                                        </td>
                                    ...
                                    @endif
                                    <td><a href="{{$item->host_url}}" target="_blank">@lang('Start Meeting')</a></td>
                                    <td><a href="{{$item->join_url}}" target="_blank">@lang('Join Meeting')</a></td>
                                    <td data-label="@lang('Action')">
                                        <a href="" data-route="{{ route('doctor.appointment.view',$item->id) }}"
                                           data-resourse="{{ $item }}" data-toggle="modal" data-target="#viewModal"
                                           class="icon-btn bg--success ml-1 viewBtn">
                                            <i class="la la-eye"></i>
                                        </a>
                                        @if ($item->p_status == 0)
                                            <a href="" data-route="{{ route('doctor.appointment.remove',$item->id) }}"
