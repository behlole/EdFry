...
class ManageUsersController extends Controller
{
    public function allUsers(){
        ...
        $staff = User::latest()->paginate(getPaginate());
        return view('admin.user.list', compact('page_title', 'empty_message', 'staff'));
    }
    ...
    public function detail($id)
    {
        ...
        $new_appointment = Appointment::where('user_id',$user->id)->where('try',1)->where('d_status',0)->where('is_complete',0)->whereHas('relationStaff', function ($query) {
            $query->where('status',1);
        })->count();

        $appointment_done = Appointment::where('user_id',$user->id)->where('try',1)->where('is_complete',1)->where('p_status',1)->where('d_status',0)->latest()->whereHas('relationUser', function ($query) {
            $query->where('status',1);
       })->count();

        $total_appointment = Appointment::where('user_id',$user->id)->where('try',1)->where('d_status',0)->latest()->where('is_complete',0)->whereHas('relationStaff', function ($query) {
            $query->where('status',1);
       })->count();
        return view('admin.user.detail', compact('page_title', 'user','new_appointment','appointment_done','total_appointment'));
    }
    ...
    public function search(Request $request, $scope)
    {
        ...
        $staff = User::where(function ($staff) use ($search) {
    ...
    {
        $staff = User::findOrFail($id);
        $page_title = 'User Login History - ' . $staff->username;
        $empty_message = 'No user login found.';
        $login_logs = $staff->login_logs()->latest()->paginate(getPaginate());
        return view('admin.user.logins', compact('page_title', 'empty_message', 'login_logs'));
    }
    ...
    public function loginHistory(Request $request)
    {
        if ($request->search) {
            ...
            return view('admin.user.logins', compact('page_title', 'empty_message', 'search', 'login_logs'));
        }
    ...
    public function showEmailSingleForm($id)
    {
        ...
        return view('admin.user.email_single', compact('page_title', 'staff'));
    }
    ...
    public function showEmailAllForm()
    {
        ...
        return view('admin.user.email_all', compact('page_title'));
    }
    ...
    public function sendEmailAll(Request $request)
    {
        ...
        foreach (User::where('status', 1)->cursor() as $staff) {
            send_general_email($staff->email, $request->subject, $request->message, $staff->username);
        }
