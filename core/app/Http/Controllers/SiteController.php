...
class SiteController extends Controller
{
    ...
    {
        try {
            $zoomMeetingController = new ZoomMeetingController();
            $data["topic"] = "TestMeeting";
            $data["duration"] = "10 minutes";
            $data["agenda"] = "Testing";
            $data["start_time"] = Carbon::now()->addDays(5);
            $data["host_video"] = "1";
            $data["participant_video"] = "1";
//            $meeting_data = $zoomMeetingController->create($data);
//            dd($meeting_data);
    ...
    {
        $page_title = "Register";
        return view($this->activeTemplate . "register", compact("page_title"));
    }
      ...
      public function appointmentStore(Request $request, $id)
    {
        ...
        $zoom_data['duration'] = $doctor->duration;
        $zoom_data['host_email'] = $doctor->email;
        $zoom_data['attendee_email'] = $request->email;
        $request->time_serial = substr_replace($request->time_serial, ' ', 5, 1);
        $time = date("H:i", strtotime($request->time_serial));
        $zoom_data['time'] = $time;
        $zoom_data['booking_date'] = $request->booking_date;
        $zoom_data['start_date'] = Carbon::parse(date('Y-m-d H:i:s', strtotime("$request->booking_date $time")));
